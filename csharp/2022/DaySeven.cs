...
public class DaySeven : ExerciseBase
{
    ...
    public DaySeven() : base(2022, 7)
    {
        _fileSystem = new(Input);
    }
    ...
    private class FileSystem
    {
        ...
        private IReadOnlyList<string> TerminalOutput { get; }

        public FileSystem(IReadOnlyList<string> terminalOutput)
        {
        ...
        private void ChangeDirectory(string parameter)
        {
            switch (parameter)
            {
                ...
                case "..":
                    DebugOutput(
                        $"Directory changed from '{CurrentDirectory?.Name}' to '{CurrentDirectory?.Parent?.Name}'");
